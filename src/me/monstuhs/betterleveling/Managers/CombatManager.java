...
public class CombatManager {
    ...
    public double getChanceToDodge(int playerLevel) {
        return Math.min((playerLevel * _chanceToDodgePerLevel), _maxDodgeChance);
    }

    public double getChanceToCrit(int playerLevel) {
    ...
    public int getDamageAfterDodgeAttempt(Player defender, int originalDamage) {
        ...
        if (doesPlayerDodge) {
            ...
            PlugginLogging.SendDodgeMessageToPlayer(defender, newDamage, originalDamage);
        }
        return newDamage;
    }
    
    public int getDamageAfterCritAttempt(Player attacker, int originalDamage){
        double roll = new Random().nextInt(100);
        boolean doesPlayerCrit =  roll <= getChanceToCrit(attacker.getLevel());        
        if(doesPlayerCrit){
            PlugginLogging.SendCritMessageToPlayer(attacker, _critDamageModifier);
            originalDamage *= _critDamageModifier;            
        }
        return originalDamage;
    }
    
    public int getDamageAfterHeadshotAttempt(Arrow arrow, LivingEntity defender, int originalDamage){
...
//        double zOffset = pointOfContact.getZ() - eyeLocation.getZ();
        ...
        if (Math.abs(yOffset) >= 0.75) {
            ...
            PlugginLogging.SendHeadShotMessageToPlayer((Player)arrow.getShooter(), _headshotDamageModifier);
